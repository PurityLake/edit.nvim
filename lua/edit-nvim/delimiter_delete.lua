...
function M.delimiter_one_delete(delim)
...
function M.delimiter_two_delete(delim1, delim2)
	local current_line = vim.api.nvim_get_current_line()
	local row, col = unpack(vim.api.nvim_win_get_cursor(0))
	local new_line = ""
	...
	new_line = current_line:sub(1, start_idx + delim1:len() - 1)
		.. current_line:sub(end_idx + delim2:len() - 1, current_line:len())
